...
class CityLoginModel extends Model
{
    ...
    public function getRevenueDataInCityDivision($division_id, $city_id)
    {
        $sql = "SELECT * FROM revenue_collection WHERE division_id = '$division_id' AND city_id = '$city_id' ORDER BY id DESC LIMIT 0, 1;";
        $result = $this->db->query($sql);
        $return = $result->getRow();
        return $return;
    }

    public function insertRevenueCollection($rev_no_of_bill_generated, $rev_nos_bill_distributed, $rev_incentive_paid_to_jalsathi, $rev_total_revenue_collected, $rev_revenue_collected_by_jalasathi, $division_id, $city_id, $userlogid)
    {
        $sql = "INSERT INTO revenue_collection(no_bill_generated,no_bill_distributed,incentive_paid_to_jalasathi,total_revenue_collected,revenue_collected_by_jalasathi,city_user_id,last_update,division_id,city_id) VALUES ('$rev_no_of_bill_generated','$rev_nos_bill_distributed','$rev_incentive_paid_to_jalsathi','$rev_total_revenue_collected','$rev_revenue_collected_by_jalasathi','$userlogid',NOW(),'$division_id','$city_id')";
        $result = $this->db->query($sql);
        // print_r($sql);die;
        return $result;
        $this->db->close();
    ...
    public function getRevColCityOnId($city_revenue_id)
    {
        $sql = "SELECT * FROM revenue_collection WHERE id = '$city_revenue_id';";
        $result = $this->db->query($sql);
        $return = $result->getRow();
        return $return;
        $this->db->close();
    ...
    public function editCityuserRevenueCollection($revenue_id, $rev_no_of_bill_generated, $rev_nos_bill_distributed, $rev_incentive_paid_to_jalsathi, $rev_total_revenue_collected, $rev_revenue_collected_by_jalasathi)
    {
        $sql = "UPDATE revenue_collection SET no_bill_generated = '$rev_no_of_bill_generated',no_bill_distributed = '$rev_nos_bill_distributed',incentive_paid_to_jalasathi = '$rev_incentive_paid_to_jalsathi',total_revenue_collected = '$rev_total_revenue_collected',revenue_collected_by_jalasathi = '$rev_revenue_collected_by_jalasathi',last_update = NOW() WHERE id ='$revenue_id';";
        $result = $this->db->query($sql);
        return $result;
        $this->db->close();
    }
