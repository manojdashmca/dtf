...
class Model extends BaseModel {
    ...
    protected function objectToRawArray($data, bool $onlyChanged = true, bool $recursive = false): ?array {
        ...
        if (
                method_exists($data, 'toRawArray') && (!empty($properties) && !empty($this->primaryKey) && !in_array($this->primaryKey, $properties, true) && !empty($data->{$this->primaryKey}))
        ) {
    ...
    ///-------------------userdefind methods-------------
    public function createRecordInTable($data, $tablename) {
        try {
            $sql = $this->createInsertQuery($data, $tablename);
            $this->db->query($sql);
            $insid = $this->db->insertID();
        } catch (Exception $e) {
            $insid=0;
        }
        $moderator = session()->get('userid');
        $trnid = session()->get('trnid');
        $sql = "INSERT INTO `action_audit` (`aa_id`, `moderator`, `action`, `table_name`, `condition_statement`, `affected_row`, `transactionid`, `transaction_time`) "
                . "VALUES (NULL, '$moderator', 'Insert', '$tablename', 'Insertid-$insid','1', '$trnid', current_timestamp());";
        $this->db->query($sql);
        return $insid;
    }

    public function updateRecordInTable($data, $tablename, $fieldname, $fieldvalue, $sign = NULL) {
        $moderator = session()->get('userid');
        $trnid = session()->get('trnid');
        $sql = $this->createUpdateQuery($data, $tablename, $fieldname, $fieldvalue, $sign);
        $this->db->query($sql);
        $affectedrow = $this->db->affectedRows();
        $sql = "INSERT INTO `action_audit` (`aa_id`, `moderator`, `action`, `table_name`, `condition_statement`, `affected_row`, `transactionid`, `transaction_time`) "
                . "VALUES (NULL, '$moderator', 'Update', '$tablename', '$fieldname/$fieldvalue($sign)','$affectedrow', '$trnid', current_timestamp());";
        $this->db->query($sql);
        return $affectedrow;
    }

    public function getTableData($fields, $table, $whereclause = 1) {
        $sql = "SELECT $fields "
                . "FROM $table "
                . "WHERE $whereclause";
    ...
    public function getTableMultyData($fields, $table, $whereclause = 1, $otherclause = '') {
        $sql = "SELECT $fields "
                . "FROM $table "
                . "WHERE $whereclause $otherclause";
        $result = $this->db->query($sql);
        $return = $result->getResult();
        return $return;
    }

    public function checkFieldDataExists($id, $table, $field) {
        $return = false;
        $sql = "SELECT count(1) as count FROM $table WHERE $field='$id'";
        $result = $this->db->query($sql);
        $resultset = $result->getRow();
    ...
    public function createInsertQuery($data, $tabale) {
        $field = array();
        $values = array();
        ...
        $sql = "INSERT INTO `" . $tabale . "` (" . $fields . ") VALUES (" . $impvalues . ")";
        return $sql;
    }

    public function createUpdateQuery($data, $table, $primarykey, $keyvalue, $sign = '=') {
        $values = array();
        foreach ($data as $key => $value) {
        ...
        switch ($sign) {
            case "=":
                $sql = "UPDATE `" . $table . "` SET " . $impvalues . " WHERE `" . $primarykey . "` = '" . $keyvalue . "'";
                break;
            case "in":
                $sql = "UPDATE `" . $table . "` SET " . $impvalues . " WHERE `" . $primarykey . "` in( " . $keyvalue . ")";
                break;
            default:
                $sql = "UPDATE `" . $table . "` SET " . $impvalues . " WHERE `" . $primarykey . "` = '" . $keyvalue . "'";
                break;
